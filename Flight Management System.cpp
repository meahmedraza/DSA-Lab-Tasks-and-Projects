...
class passenger{
	private:
		...
		const int eseats;
	protected:
		string first_name,last_name, phone_no, meal_type, Visa, address;
		int Bus, eco, date_dep, deposit;
		char start[30];
		char destination[30];
	public:		
		passenger(string f_name, string l_name, string phone_no, string add="N/A", string pass_no="N/A", int dd=000000): b_seats(50), eseats(250)
		{
			date_dep=dd;
			first_name=f_name;
			last_name=l_name;
...
class booking:public passenger
{
	...
	bool ldest()
	{
		...
		else
		{
			for(i=0;i<5;i++)
			{
				if(strcmp(l_destination[i],start)==0)
					n++;
			}
			for(j=0;j<5;j++)
			{
			...
			if(m==1&&n==1)
			{
				deposit = lpayment(start, destination);
				cout<<endl<<"Price: "<<deposit<<" Pkr/seat"<<endl;
				return true;
			}
			else 
				return false;
		...
		bool idest(string pass)
	{
		...
		else
		{
			for(i=0;i<11;i++)
			{
				if(strcmp(i_destination[i],start)==0)
					n++;
			}
			for(j=0;j<11;j++)
			{
			...
			if(m==1&&n==1)
			{
				deposit = ipayment(start, destination);
				cout<<endl<<"Price: "<<deposit<<" Pkr/seat"<<endl;
				return true;
			}
			else 
				...
				if(Bus>0&&eco>0)
				{
					...
					switch(k)
					{
						case 49:
						{
							...
							cin>>NoOfSeats;
							deposit=(deposit*NoOfSeats)+5000;
							if(NoOfSeats<=Bus)
							{
		...
		void Registration()
		{
			...
			if(k==49)
			{
				...
				<<"Phone: "<<address<<endl
				<<"Address: "<<phone_no<<endl
				<<"Passport Number: "<<Visa<<endl
				<<"Origin: "<<start
				<<endl<<"Destination: "<<destination
				<<endl<<"Food: "<<meal_type
				<<endl<<"Number of Seats: "<<NoOfSeats
				<<endl<<"Class: Business"
				<<endl<<"Date of departure: "<<date_dep
				<<endl<<"Time: 4.00 PM"
				<<endl<<"Ticket: HF"<<tkt
				<<endl<<"Bill(in pakistani rupees): "<<deposit<<"/-"<<endl;				
				outfile.close();
			}
			else if(k==50)
			{
		...
		inline void drive()
		{
			...
			cout<<endl<<"Drop Off: "<<start<<" Airport"
			<<endl<<"Fare: 300/-";
...
int main()
{
	...
	int choice, date_dep, date, month, year;
	bool ans;
	char ch;
	time_t t= time(NULL);
		...
		case 49:
			{
				...
				booking obj1(f_name,l_name,phone_no,add,pass_no,date_dep);
				ptr = &obj1;
				back:
			...
			case 51:	//choice = 3
				{
					...
					cout<<"Enter Your Last Name: ";
					getline(cin,l_name);
					cout<<"Enter Your phone_no Num(Without -): ";
					cin>>phone_no;
...
bool datecheck(int date_dep, int date, int month, int year)
{
	if((date_dep%10000)>2017&&(date_dep%10000)==year)
		if(((date_dep/10000)%100)>0 && ((date_dep/10000)%100)<13&&((date_dep/10000)%100)==month)
			if((date_dep/1000000)>0 && (date_dep/1000000)<32&&(date_dep/1000000)>date&&(date_dep/1000000)<(date+20))
				return true;
	else
		return false;
}
